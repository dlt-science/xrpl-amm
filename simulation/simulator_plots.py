...
class SimulatorResults():
    def __init__(self, sim, assetA, assetB, external_prices, gbm_mu, gbm_sigma, xrpl_block_conf, eth_block_conf, xrpl_fees, eth_fees, safe_profit_margin, max_slippage, iterations):
        self.sim = sim
    ...
    def save_simulation_data(self, foldername):
        ...
        with open(os.path.join(os.path.join("simulation_results", foldername), "info.txt"), "w") as f:
    ...
    def plot_arbitrageurs_profits(self, ax, set_title=True):
        xrpl_profits_sum = [sum(self.sim["xrpl_profits_total"][i])
        ...
        ax.plot(self.x_axis, xrplCAM_profits_sum, label='XRPL-AMM-CAM')
        ax.plot(self.x_axis, xrpl_profits_sum, label='XRPL-AMM')
        ax.plot(self.x_axis, uniswap_profits_sum, label='Uniswap')
    ...
    def plot_average_arbitrageurs_profits(self, ax, set_title=True):
        ...
        x = ['XRPL-AMM-CAM', 'XRPL-AMM', 'Uniswap']
    ...
    def plot_number_of_txs(self, ax, set_title=True):
        ax.plot(self.x_axis,
                self.sim["xrplCAM_arbit_txs_total"], label='XRPL-AMM-CAM')
        ax.plot(self.x_axis,
                self.sim["xrpl_arbit_txs_total"], label='XRPL-AMM')
        ax.plot(self.x_axis,
                self.sim["uniswap_arbit_txs_total"], label='Uniswap')

        ax.set_xlabel('Iteration #')
        ax.set_ylabel('Number of Transactions')
        ax.set_title(
            'Number of Txs made by the arbitrageur at each iteration') if set_title else None
        ax.legend()
        ax.yaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.0f}'))
        return ax
    ...
    def plot_average_total_txs(self, ax, set_title=True):
        ...
        avg_xrplCAM_txs = round(np.average(
            self.sim["xrplCAM_arbit_txs_total"]))
        ...
        ax.text(x[0], y[0]-2, '(~'+f'{self.compute_losses(self.sim["xrplCAM_profits_total"])} losses)',
                ha='center', va='top', fontsize=8)

        ax.text(x[1], y[1]-2, '(~'+f'{self.compute_losses(self.sim["xrpl_profits_total"])} losses)',
                ha='center', va='top', fontsize=8)

        ax.text(x[2], y[2]-2, '(~'+f'{self.compute_losses(self.sim["uniswap_profits_total"])} losses)',
                ha='center', va='top', fontsize=8)
        ax.bar_label(ax.containers[0], fmt='{:,.0f}')
    ...
    def plot_total_fees_paid(self, ax, set_title=True):
        xrpl_fees_total = [
            i*self.xrpl_fees for i in self.sim["xrpl_arbit_txs_total"]]
        xrplCAM_fees_total = [
            i*self.xrpl_fees for i in self.sim["xrplCAM_arbit_txs_total"]]
        uniswap_fees_total = [
            i*self.eth_fees for i in self.sim["uniswap_arbit_txs_total"]]

        ax.plot(self.x_axis, xrplCAM_fees_total, label='XRPL-AMM-CAM')
        ax.plot(self.x_axis, xrpl_fees_total, label='XRPL-AMM')
        ax.plot(self.x_axis, uniswap_fees_total, label='Uniswap')

        ax.set_xlabel('Iteration #')
        ax.set_ylabel('Cumulative Tx fees ({})'.format(self.assetB))
        ax.set_title(
            'Total Tx fees paid by the arbitrageur at each iteration') if set_title else None
        ax.legend()
        ax.yaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.0f}'))
        return ax
    ...
    def plot_average_total_fees(self, ax, set_title=True):
        ...
        x = ['XRPL fees for \n ~'+f'{avg_xrplCAM_txs} txs', 'XRPL fees for \n ~' +
    ...
    def plot_amm_prices(self, ax, key, label, set_title=True):
        ...
        ax.plot(self.sim[key][0][1],
                linestyle='dashed', label=label)
        ax.set_xlabel('Time Step')
        ax.set_ylabel('Price')
        ax.set_title(
            'Price Evolution (last iteration only)') if set_title else None
        ax.legend()
        return ax

    def plot_cdf(self, ax, times1, times2, times3, bin, xlabel, multiplier=1, set_title=True):
        sns.set()
        ...
        ax.plot(bins_count1[1:], cdf1, color="r",
                linewidth=3, label="XRPL-AMM-CAM")
        ax.plot(bins_count2[1:], cdf2, color="g",
                linewidth=3, label="XRPL-AMM")
        ax.plot(bins_count3[1:], cdf3, color="b", linewidth=3, label="Uniswap")
    ...
    def plot_prices_moving_average(self, ax, set_title=True):
        percent_differences_uni_total, percent_differences_xrpl_total, percent_differences_xrplCAM_total = [], [], []
        for i in range(self.iterations):
            percent_differences_xrplCAM_total.append(
                [(b - a) / a * 100 for a, b in zip(self.external_prices, self.sim['xrplCAM_sps_total'][i][1])])
            percent_differences_xrpl_total.append(
                [(b - a) / a * 100 for a, b in zip(self.external_prices, self.sim['xrpl_sps_total'][i][1])])
            percent_differences_uni_total.append(
                [(b - a) / a * 100 for a, b in zip(self.external_prices, self.sim['uniswap_sps_total'][i][1])])

        averages_uni = [sum(values)/len(values)
                        for values in zip(*percent_differences_uni_total)]
        averages_xrpl = [sum(values)/len(values)
                         for values in zip(*percent_differences_xrpl_total)]
        averages_xrplCAM = [sum(values)/len(values)
        ...
        ax.plot(indices[: -window_size + 1], smoothed_percent_differences_uni, marker='',
                linestyle='-', linewidth=1, label='Uniswap ({}MA)'.format(window_size), color="b")
        ax.plot(indices[: -window_size + 1], smoothed_percent_differences_xrplCAM, marker='',
                linestyle='-', linewidth=1, label='XRPL-AMM-CAM ({}MA)'.format(window_size), color="r")
        ax.plot(indices[: -window_size + 1], smoothed_percent_differences_xrpl, marker='',
                linestyle='-', linewidth=1, label='XRPL-AMM ({}MA)'.format(window_size), color="g")
    ...
    def plot_LP_returns(self, ax, set_title=True):
        avg_B_xrplCAM = sum(self.sim['xrplCAM_tfees_total']
                            ) / len(self.sim['xrplCAM_tfees_total'])
        avg_B_xrpl = sum(self.sim['xrpl_tfees_total']
                         ) / len(self.sim['xrpl_tfees_total'])
        avg_B_uniswap = sum(self.sim['uniswap_tfees_total']
                            ) / len(self.sim['uniswap_tfees_total'])

        df = pd.DataFrame({
            'Categories': ['XRPL-AMM-CAM', 'XRPL-AMM', 'Uniswap'],
        ...
        bar_width = 0.4
        bars_bottom = ax.bar(df['Categories'], [
                             avg_B_xrplCAM, avg_B_xrpl, avg_B_uniswap], color=cmap(0.7), label='Trading fee returns')
        bars_top = ax.bar(df['Categories'], [(sum(self.sim['bids_profit_total'][i]['B_adjusted'] for i in range(len(self.sim['bids_profit_total'])))/self.iterations), 0, 0],
                          bottom=[avg_B_xrplCAM, avg_B_xrpl, avg_B_uniswap], color=cmap(0.9), label='CAM bids returns')
    ...
    def display_results(self, to_save=False, foldername=None):
        ...
        if not to_save:
            print('Arbitrageurs are more profitable on XRPL',
                  f'{self.sim["xrpl_ArbProfits_advantage"]}%', 'of the time')
            print('Arbitrageurs are more profitable on XRPL_CAM',
                  f'{self.sim["xrplCAM_ArbProfits_advantage"]}%', 'of the time')
            print('Arbitrageurs are more profitable on XRPL_CAM than XRPL',
                  f'{self.sim["xrpls_CAM_ArbProfits_advantage"]}%', 'of the time')
            print('---')
            print('Price Sync. is better on XRPL',
                  f'{self.sim["xrpl_PriceGap_advantage"]}%', 'of the time')
            print('Price Sync. is better on XRPL_CAM',
                  f'{self.sim["xrplCAM_PriceGap_advantage"]}%', 'of the time')
            print('Price Sync. is better on XRPL_CAM than XRPL',
                  f'{self.sim["xrpls_CAM_PriceGap_advantage"]}%', 'of the time')
            print('---')
            print('LPs earn more on XRPL',
                  f'{self.sim["xrpl_LP_returns_advantage"]}%', 'of the time')
            print('LPs earn more on XRPL_CAM',
                  f'{self.sim["xrplCAM_LP_returns_advantage"]}%', 'of the time')
            print('LPs earn more on XRPL_CAM than XRPL',
                  f'{self.sim["xrpls_CAM_LP_returns_advantage"]}%', 'of the time')

        fig, ((ax1, ax2, ax3), (ax4, ax5, ax6), (ax7, ax8, ax9), (ax10, ax11, _), (ax13, ax14, ax15), (ax16, _, _)
              ) = plt.subplots(6, 3, figsize=(20, 30))
        fig.text(0.37, 0.9, s='XRPL fees: '+f'{self.xrpl_fees}'+', ETH fees: '+f'{self.eth_fees}'+', Safe profit margin: ' +
                 f'{self.safe_profit_margin}%'+',  Max. slippage: '+f'{self.max_slippage}%', ha='right', va='top')
        ...
        self.plot_amm_prices(ax7, 'xrplCAM_sps_total', 'XRPL-AMM-CAM')
        self.plot_amm_prices(ax8, 'xrpl_sps_total', 'XRPL-AMM')
        self.plot_amm_prices(ax9, 'uniswap_sps_total', 'Uniswap')

        percent_differences_uniswap_total, percent_differences_xrpl_total, percent_differences_xrplCAM_total = [], [], []
        for i in range(self.iterations):
            percent_differences_xrplCAM_total.append(
                [(b - a)/a * 100 for a, b in zip(self.external_prices, self.sim['xrplCAM_sps_total'][i][1])])
            percent_differences_xrpl_total.append(
            ...
            percent_differences_uniswap_total.append(
                [(b - a)/a * 100 for a, b in zip(self.external_prices, self.sim['uniswap_sps_total'][i][1])])
        self.plot_cdf(ax10, np.abs(percent_differences_xrplCAM_total), np.abs(
            percent_differences_xrpl_total), np.abs(percent_differences_uniswap_total), 500, 'percent diff (%)')

        ax11 = plt.subplot2grid((6, 3), (3, 1), colspan=2)
        self.plot_prices_moving_average(ax11)

        slips_xrpl_cam, slips_xrpl, slips_uniswap = [], [], []
        for i in range(self.iterations):
            slips_xrpl_cam += self.sim['xrplCAM_slippages_total'][i]
            slips_xrpl += self.sim['xrpl_slippages_total'][i]
            slips_uniswap += self.sim['uniswap_slippages'][i]
        self.plot_cdf(ax13, slips_xrpl_cam, slips_xrpl,
                      slips_uniswap, 500, 'Slippage (%)', 100)

        self.plot_cdf(ax14, self.sim['impermanent_losses']['xrplCAM'], self.sim['impermanent_losses']
                      ['xrpl'], self.sim['impermanent_losses']['uniswap'], 500, 'Impermanent Loss (%)', 100)
        ...
        if to_save:
            folder_path = os.path.join(
                "simulation_results", foldername, "plots")
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)

            fig.savefig(os.path.join(
                folder_path, 'figure_plots_combined.pdf'), format='pdf')
            plt.close()
        else:
            plt.show()
    ...
    def save_plots_results(self, foldername, set_title):
        ...
        plots_dict = {"arbit_profits.pdf": lambda ax: self.plot_arbitrageurs_profits(ax, set_title),
                      "avg_arbit_profits.pdf": lambda ax: self.plot_average_arbitrageurs_profits(ax, set_title),
                      "numb_of_txs.pdf": lambda ax: self.plot_number_of_txs(ax, set_title),
                      "avg_numb_of_txs.pdf": lambda ax: self.plot_average_total_txs(ax, set_title),
                      ...
                      "xrplCAM_price_sync_last_iter.pdf": lambda ax: self.plot_amm_prices(ax, 'xrplCAM_sps_total', 'XRPL-AMM-CAM', set_title),
                      "xrpl_price_sync_last_iter.pdf": lambda ax: self.plot_amm_prices(ax, 'xrpl_sps_total', 'XRPL-AMM', set_title),
                      "uniswap_price_sync_last_iter.pdf": lambda ax: self.plot_amm_prices(ax, 'uniswap_sps_total', 'Uniswap', set_title),
                      "cdf_price_diff.pdf": lambda ax: self.plot_cdf(ax, np.abs(percent_differences_xrplCAM_total), np.abs(percent_differences_xrpl_total), np.abs(percent_differences_uniswap_total), 500, 'percent diff (%)', set_title=set_title),
                      "prices_MA.pdf": lambda ax: self.plot_prices_moving_average(ax, set_title),
                      "cdf_slippages.pdf": lambda ax: self.plot_cdf(ax, slips_xrpl_cam, slips_xrpl, slips_uniswap, 500, 'Slippage (%)', 100, set_title),
        ...
        for filename in plots_dict:
            fig, ax = plt.subplots(
                figsize=(15, 6)) if filename == 'prices_MA.pdf' else plt.subplots()
