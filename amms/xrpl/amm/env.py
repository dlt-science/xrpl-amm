...
class AMMi:
    ...
    def __init__(self, ammi):
        ...
        self.AuctionSlot = {"slot_owner": None, "t": 0, "discountedFee": 0, "price": 0}
        self.MinSlotPrice = 0

    def update_MinSlotPrice(self):
        self.MinSlotPrice = self.assets["LPTokens"] * self.TFee / 25

    def add_asset(self, asset: str, amount: float):
        if asset in self.assets:
            self.assets[asset] += amount
            if asset == "LPTokens":
                self.update_MinSlotPrice()
        else:
            self.assets[asset] = amount
    ...
    def delta_token_Double(self, LPTokens_In_or_Out: float, bal_token: float) -> float:
        ...
        delta_token = (LPTokens_In_or_Out / self.ammi.assets["LPTokens"]) * bal_token
    ...
    def delta_LPTokens_Single(
        self, amount: float, bal_token: float, TFee: float
    ) -> float:
        # bal_token = balance of asset in pool (bal_A would be balance of asset A)
        # L = amount of LP Tokens returned
        L = self.ammi.assets["LPTokens"] * (
            (1 + (amount - TFee * (1 - self.ammi.W) * amount) / bal_token)
            ** self.ammi.W
            - 1
        )
        return L

    def delta_token_Single(
        self, bal_token: float, LPTokenOut: float, TFee: float
    ) -> float:
        return (
            ((LPTokenOut / self.ammi.assets["LPTokens"] + 1) ** (1 / self.W) - 1)
            / (1 - TFee * (1 - self.ammi.W))
        ) * bal_token
    ...
    def delta_token_WS(self, bal_token: float, LPTokenIn: float, TFee: float) -> float:
        return bal_token * (
            1
            - (1 - LPTokenIn / self.ammi.assets["LPTokens"]) ** (1 / self.W)
            * (1 - (1 - self.W) * TFee)
        )
    ...
    def delta_tokenOut_Swap(
        self, balAssetIn: float, balAssetOut: float, delta_tokenIn: float, TFee: float
    ) -> float:
        # delta_tokenIn = amount of asset to swap in
        return balAssetOut * (
            1
            - (balAssetIn / (balAssetIn + delta_tokenIn * (1 - TFee)))
            ** (self.W / self.W)
        )

    def delta_tokenIn_Swap(
        self,
        bal_tokenIn: float,
        bal_tokenOut: float,
        delta_tokenOut: float,
        TFee: float,
    ) -> float:
        ...
        return (
            bal_tokenIn
            * (
                (bal_tokenOut / (bal_tokenOut - delta_tokenOut)) ** (self.W / self.W)
                - 1
            )
            * 1
            / (1 - TFee)
    ...
    def delta_tokenIn_given_spotprices(
        self, balAssetIn: str, pre_sp: float, post_sp: float
    ) -> float:
    ...
    def avg_slippage_tokenIn(
        ...
        S_delta_tokenIn = self.slippage_slope_tokenIn(
            bal_tokenIn, TFee
        ) * self.delta_tokenIn_Swap(bal_tokenIn, bal_tokenOut, delta_tokenOut, TFee)
    ...
    def avg_slippage_tokenOut(
        self, bal_tokenIn: float, bal_tokenOut: float, delta_tokenIn: float, TFee: float
    ) -> float:
        S_delta_tokenOut = self.slippage_slope_tokenOut(
            bal_tokenIn
        ) * self.delta_tokenOut_Swap(bal_tokenIn, bal_tokenOut, delta_tokenIn, TFee)
    ...
    def spot_price(self, assetIn: str, assetOut: str) -> float:
        sp = (
            (self.ammi.assets[assetIn] / self.ammi.W)
            / (self.ammi.assets[assetOut] / self.ammi.W)
            * 1
            / (1 - self.ammi.TFee)
        )
        return sp
    ...
    def spot_price(self, asset: str, other_asset: str, discounted_fee=False) -> float:
        if discounted_fee:
            TFee = 0
        else:
            TFee = self.TFee
        sp = (
            (self.assets[other_asset] / self.W)
            / (self.assets[asset] / self.W)
            * 1
            / (1 - TFee)
        )
        return sp
...
class AMMVote(AMMi):
    ...
    def vote_entry(self, fee_val: float):
        # fee_val = trading fee the user (liquidity provider) wants to vote for (between 0% and 1%)
        assert 0 <= fee_val <= 0.01
        assert self.user.assets["LPTokens"] > 0

        vote_weight = self.user.assets["LPTokens"] / self.ammi.assets["LPTokens"]
        vote_slot = {
        ...
        if len(self.ammi.VoteSlots) == 0:
            ...
            for i in range(len(self.ammi.VoteSlots)):
                ...
                if len(self.ammi.VoteSlots) < 8:
                    ...
                    if (
                        self.ammi.VoteSlots[min_vweight_index]["vote_weight"]
                        < vote_weight
                    ):
    ...
    def monitor_VoteSlots(self):
        for i in range(len(self.ammi.VoteSlots)):
            if self.user.user_name == self.ammi.VoteSlots[i]["user"]:
                ...
                # update vote entries
                for i in range(len(self.ammi.VoteSlots)):
                    self.ammi.VoteSlots[i]["vote_weight"] = (
                        self.ammi.LPs[self.ammi.VoteSlots[i]["user"]]
