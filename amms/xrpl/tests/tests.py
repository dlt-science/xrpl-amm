...
@pytest.mark.parametrize("amount1,amount2", [(10, 10), (232, 52454), (23, 45)])
def test_createAMM(alice, asset1, asset2, amount1, amount2):
    ...
    if alice.assets[asset1] >= amount1 and alice.assets[asset2] >= amount2:
        amm = alice.createAMM(
            ammID=1, asset1=asset1, asset2=asset2, amount1=amount1, amount2=amount2
        )
...
@pytest.mark.parametrize("LPTokenOut", [(15), (20), (30), (343)])
...
@pytest.mark.parametrize("amount1,amount2", [(232, 433), (15, 20), (78, 12)])
def test_deposit_Amount1_Amount2(
    amm, alice, asset1, asset2, amount1, amount2, prev_bal
):
    alice_deposits = Deposit(alice, amm)
...
@pytest.mark.parametrize(
    "asset,amount", [("asset1", 20), ("asset2", 30), ("asset1", 233)]
)
def test_deposit_Amount(asset, amm, alice, amount, prev_bal, request):
    ...
    if alice.assets[request.getfixturevalue(asset)] > amount:
        ...
        assert (
            amm.assets[request.getfixturevalue(asset)]
            == prev_bal["amm"][request.getfixturevalue(asset)] + amount
        )
        assert (
            alice.assets[request.getfixturevalue(asset)]
            == prev_bal["alice"][request.getfixturevalue(asset)] - amount
        )
        assert "alice" in amm.LPs
    else:
        with pytest.raises(Exception) as e:
...
@pytest.mark.parametrize(
    "asset,amount,LPTokenOut",
    [("asset1", 20, 10), ("asset2", 30, 4), ("asset1", 233, 23423), ("asset2", 5, 13)],
)
def test_deposit_Amount_LPTokenOut(
    alice, amm, asset, amount, LPTokenOut, prev_bal, request
):
    alice_deposits = Deposit(alice, amm)
    if alice.assets[request.getfixturevalue(asset)] > amount:
        ...
        assert (
            alice.assets[request.getfixturevalue(asset)]
            == prev_bal["alice"][request.getfixturevalue(asset)] - delta_token
        )
        assert (
            amm.assets[request.getfixturevalue(asset)]
            == prev_bal["amm"][request.getfixturevalue(asset)] + delta_token
        )
    else:
        with pytest.raises(Exception) as e:
            alice_deposits.deposit_Amount(
...
@pytest.mark.parametrize("LPTokenIn", [(15), (20), (30), (343), (25435.435)])
def test_withdraw_LPTokenIn(amm, alice, prev_bal, asset1, asset2, LPTokenIn):
    ...
    if alice.assets["LPTokens"] >= LPTokenIn:
        delta_asset1 = delta_token_Double(
            LPTokens_In_or_Out=LPTokenIn,
            bal_token=amm.assets[asset1],
            bal_LPTokens=amm.assets["LPTokens"],
        )
        delta_asset2 = delta_token_Double(
            LPTokens_In_or_Out=LPTokenIn,
            bal_token=amm.assets[asset2],
            bal_LPTokens=amm.assets["LPTokens"],
        )
...
@pytest.mark.parametrize("amount1,amount2", [(232, 433), (15, 20), (78, 12), (3, 5)])
def test_withdraw_Amount1_Amount2(
    amm, alice, asset1, asset2, amount1, amount2, prev_bal
):
    alice_withdrawals = Withdraw(alice, amm)
    # Z = LPTokensIn
...
def test_withdraw_Amount(amm, alice, prev_bal, amount, asset, request):
    ...
    if (
        amm.assets[request.getfixturevalue(asset)] > amount
        and alice.assets["LPTokens"] >= L
    ):
...
@pytest.mark.parametrize(
    "asset,amount,LPTokenIn",
    [("asset2", 30, 4), ("asset1", 423, 2423), ("asset2", 5, 13), ("asset1", "", 32)],
)
def test_withdraw_Amount_LPTokenIn(
    alice, amm, asset, amount, LPTokenIn, prev_bal, request
):
    alice_withdrawals = Withdraw(alice, amm)
    if alice.assets["LPTokens"] >= LPTokenIn:
        # Y = amount of asset A
        ...
        if (amount and Y >= amount) or (not amount):
            ...
            assert (
                alice.assets[request.getfixturevalue(asset)]
                == prev_bal["alice"][request.getfixturevalue(asset)] + Y
            )
            assert (
                amm.assets[request.getfixturevalue(asset)]
                == prev_bal["amm"][request.getfixturevalue(asset)] - Y
            )
        else:
            with pytest.raises(Exception) as e:
                alice_withdrawals.withdraw_Amount_LPTokenIn(
...
def test_swap_given_amount_In(
    amm, alice, assetIn, assetOut, amountIn, prev_bal, request
):
    alice_swaps = Swap(alice, amm)
    delta_tokenOut = delta_tokenOut_Swap(
        bal_assetIn=amm.assets[request.getfixturevalue(assetIn)],
        bal_assetOut=amm.assets[request.getfixturevalue(assetOut)],
        delta_tokenIn=amountIn,
    )
    if (
        alice.assets[request.getfixturevalue(assetIn)] >= amountIn
        and amm.assets[request.getfixturevalue(assetOut)] > delta_tokenOut
    ):
        alice_swaps.swap_given_amount_In(
            assetIn=request.getfixturevalue(assetIn),
            assetOut=request.getfixturevalue(assetOut),
            amountIn=amountIn,
        )
        assert (
            amm.assets[request.getfixturevalue(assetIn)]
            == prev_bal["amm"][request.getfixturevalue(assetIn)] + amountIn
        )
        assert (
            amm.assets[request.getfixturevalue(assetOut)]
            == prev_bal["amm"][request.getfixturevalue(assetOut)] - delta_tokenOut
        )
        assert (
            alice.assets[request.getfixturevalue(assetOut)]
            == prev_bal["alice"][request.getfixturevalue(assetOut)] + delta_tokenOut
        )
        assert (
            alice.assets[request.getfixturevalue(assetIn)]
            == prev_bal["alice"][request.getfixturevalue(assetIn)] - amountIn
        )
        # conservation function preserved: (LOOKS LIKE IT'S NOT EXACTLY PRESERVED TO THE DECIMAL POINT!)
        assert round(
            (amm.assets[request.getfixturevalue(assetIn)]) ** 0.5
            * (amm.assets[request.getfixturevalue(assetOut)]) ** 0.5,
            0,
        ) == round(prev_bal["conservation_func"], 0)
    else:
        with pytest.raises(Exception) as e:
            alice_swaps.swap_given_amount_In(
                assetIn=request.getfixturevalue(assetIn),
...
def test_swap_given_amount_Out(
    amm, alice, assetIn, assetOut, amountOut, prev_bal, request
):
    alice_swaps = Swap(alice, amm)
    if amm.assets[request.getfixturevalue(assetOut)] > amountOut:
        delta_tokenIn = delta_tokenIn_Swap(
            bal_tokenIn=amm.assets[request.getfixturevalue(assetIn)],
            bal_tokenOut=amm.assets[request.getfixturevalue(assetOut)],
            delta_tokenOut=amountOut,
        )
        if alice.assets[request.getfixturevalue(assetIn)] >= delta_tokenIn:
            alice_swaps.swap_given_amount_Out(
                assetIn=request.getfixturevalue(assetIn),
                assetOut=request.getfixturevalue(assetOut),
                amountOut=amountOut,
            )
            assert (
                amm.assets[request.getfixturevalue(assetIn)]
                == prev_bal["amm"][request.getfixturevalue(assetIn)] + delta_tokenIn
            )
            assert (
                amm.assets[request.getfixturevalue(assetOut)]
                == prev_bal["amm"][request.getfixturevalue(assetOut)] - amountOut
            )
            assert (
                alice.assets[request.getfixturevalue(assetOut)]
                == prev_bal["alice"][request.getfixturevalue(assetOut)] + amountOut
            )
            assert (
                alice.assets[request.getfixturevalue(assetIn)]
                == prev_bal["alice"][request.getfixturevalue(assetIn)] - delta_tokenIn
            )
            # conservation function preserved: (LOOKS LIKE IT'S NOT EXACTLY PRESERVED TO THE DECIMAL POINT!)
            assert round(
                (amm.assets[request.getfixturevalue(assetIn)]) ** 0.5
...
def test_swap_given_postSP(amm, alice, assetIn, assetOut, prev_bal, request, post_sp):
    alice_swaps = Swap(alice, amm)
    delta_tokenIn = delta_tokenIn_given_spotprices(
        bal_assetIn=amm.assets[request.getfixturevalue(assetIn)],
        bal_assetOut=amm.assets[request.getfixturevalue(assetOut)],
        post_sp=post_sp,
    )
    delta_tokenOut = delta_tokenOut_Swap(
        bal_assetIn=amm.assets[request.getfixturevalue(assetIn)],
        bal_assetOut=amm.assets[request.getfixturevalue(assetOut)],
        delta_tokenIn=delta_tokenIn,
    )
    if (
        amm.assets[request.getfixturevalue(assetOut)] > delta_tokenOut
        and alice.assets[request.getfixturevalue(assetIn)] > delta_tokenIn
    ):
        alice_swaps.swap_given_postSP(
            assetIn=request.getfixturevalue(assetIn),
            assetOut=request.getfixturevalue(assetOut),
            post_sp=post_sp,
        )
        ...
        assert post_sp == round(
            amm.spot_price1(
                request.getfixturevalue(assetIn), request.getfixturevalue(assetOut)
            ),
            2,
        )
    else:
...
@pytest.mark.parametrize("fee_val", [(0.008), (0.001), (0.004)])
...
def test_bid(alice, amm, t, min_price, max_price):
    ...
    if (min_price and min_price > alice.assets["LPTokens"]) or (
        max_price and max_price > alice.assets["LPTokens"]
    ):
        with pytest.raises(Exception) as e:
        ...
        assert (
            Deposit(alice, amm).TFee
            == Withdraw(alice, amm).TFee
            == Swap(alice, amm).TFee
            == 0
        )
